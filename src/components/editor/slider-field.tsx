...
const SliderField: React.FC<SliderFieldProps> = ({
  ...
  return (
    <>
      <div
        className={`flex items-center justify-between ${hasTopPadding ? 'mt-8' : ''}`}
      >
        ...
        <Input
          ...
          className={`w-12 rounded-md border border-transparent px-2 py-0.5 text-center text-sm ${
            disabled
              ...
              : 'text-muted-foreground hover:border-border hover:text-foreground hover:animate-pulse'
          }`}
          min={min}
          max={max}
          step={step}
      ...
      <Slider
        ...
        step={step}
        onValueChange={value =>
          !disabled && handleAttributeChange(attribute, value[0])
        }
